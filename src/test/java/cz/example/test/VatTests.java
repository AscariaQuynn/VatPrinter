package cz.example.test;

import cz.example.vatprinter.Application;
...
public class VatTests {
    ...
    @Test
    public void testHighestRates() {
        // Prepare mocked remote results, so it persists when source changes
        mockServer.expect(requestTo("http://jsonvat.com"))
                .andRespond(withSuccess(MOCK_RESULTS, MediaType.APPLICATION_JSON));
        // Call rest template for mocked results
        List<VatRate> rates = vatService.findRates(MOCK_RESULTS_COUNT, VatRateOrder.HIGHEST_FIRST,
                LexicographicOrder.ALPHABETICAL);
        // Verify that rest template was correctly called
        mockServer.verify();
        // Verify that correct results count is returned
        assertTrue(rates.size() == MOCK_RESULTS_COUNT);
        // Check order, so greater standard rate comes first
        if(MOCK_RESULTS_COUNT > 1) {
            for (int i = 0; i < MOCK_RESULTS_COUNT - 1; i++) {
                assertTrue(rates.get(i).getStandardRate().compareTo(rates.get(i + 1).getStandardRate()) >= 0);
            }
        }
    }
}
